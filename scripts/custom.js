...
$(document).ready(function(){      
	...
	function init_template(){
        ...
        localStorage.setItem('z_Sticky-Visited:' + window.location.pathname, 'Page Visited');
       ...
       $(".text-size-default").click(function() {$(".text-size-changer *").css("font-size", "");});
        ...
        //Local Error Message
        if (window.location.protocol === "file:"){$('a').on('mouseover',function(){console.log("You are seeing these errors because your file is on your local computer. For real life simulations please use a Live Server or a Local Server such as AMPPS or WAMPP or simulate a  Live Preview using a Code Editor like Adobe Brackets http://brackets.io (it's 100% free) or Visual Studio Code - https://code.visualstudio.com/ with Live Server https://marketplace.visualstudio.com/items?itemName=ritwickdey.LiveServer (100% free as well) - PWA functions and AJAX Page Transitions will only work in these scenarios.");});} 
        ...
        //Before and After (BA_Slider) - Added in Sticky V4.1
        var BA_slider = $('#before-after-slider');
        if(BA_slider.length){
            const sliderBA = document.getElementById('before-after-slider');
            const beforeBA = document.getElementById('before-image');
            const beforeImageBA = beforeBA.getElementsByTagName('img')[0];
            ...
            let width = sliderBA.offsetWidth;
            ...
            function slideIt(x) {
                let transform = Math.max(0, (Math.min(x, sliderBA.offsetWidth)));
